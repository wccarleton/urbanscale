...
plt1 + plt2 + plt3 + plt4 + plt5 + plt6 + plot_layout(design = layout)
...
#################### ANALYSIS FUNCTIONS #####################
# these functions just wrap a large number of commonly used options and 
# nimble function calls with switches to change relevant variables
# based on specific datasets and analses as needed. They are not really
# full functions in the sense that a number of variables are hard-coded
# within them and there's little in terms of parameters for them. If you want
# to change things about the MCMC simulation, or the Bayesian models, you 
# will likely need to modify these. They were only written to cut down on 
# redundant calls and variable assignment.

# wrapper for running the core analysis given a specific dataset, model name,
# and type (ie Roman data or Modern)
run_analysis <- function(dataset,
    ...
    return(list(mcmc_out = mcmc_out, y = y))
}

# wrapper for extracting results, plotting traces, and convergence diagnostics
gather_results <- function(y,
                        mcmc_out,
                        modelname,
                        type){
...
results <- run_analysis(dataset = RomanUrban,
                        modelname = modelname,
                        type = modeltype,
                        niter = niter,
                        nburnin = nburnin,
...
modelname = "hnwi"
results <- run_analysis(dataset = global_hnwi,
                        modelname = modelname,
                        type = modeltype,
                        niter = niter,
                        nburnin = nburnin,
...
results <- run_analysis(dataset = tall_buildings,
                        modelname = modelname,
                        type = modeltype,
                        niter = niter,
                        nburnin = nburnin,
